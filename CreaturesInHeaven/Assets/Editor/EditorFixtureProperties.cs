...
{
    private List<(FixtureDefinition def, FixtureDriver driver, FixtureProfile profile)> _selection
        = new List<(FixtureDefinition, FixtureDriver, FixtureProfile)>();
    ...
    private void WireUpCallbacks()
    {
        ...
        {
            var mode = (FixtureDefinition.ColourMode)e.newValue;
    ...
    private void RefreshColourUI()
    {
        ...
        if (allSameMode && firstDef.Colour == FixtureDefinition.ColourMode.Blackbody)
        {
            ...
            if (allSameTemp)
            {
                ...
                Color preview = FixtureDefinition.BlackbodyToRGB(firstDef.ColourTemperature);
        ...
        else if (allSameMode && firstDef.Colour == FixtureDefinition.ColourMode.RGB)
        {
    ...
    private void RefreshSelection()
    {
        ...
        {
            var def = obj.GetComponent<FixtureDefinition>();
            if (def == null) continue;

            var driver = def.GetComponent<FixtureDriver>();
