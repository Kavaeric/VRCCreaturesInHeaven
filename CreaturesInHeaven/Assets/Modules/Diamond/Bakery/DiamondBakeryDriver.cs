...
public class DiamondBakeryDriver : MonoBehaviour
{
    ...
    // animated zoom so a scrubbed/baked frame reflects the fixture's zoom.
    ...
    void Update()
    {
        ...
        // Flux-conserving intensity: as the cone opens (zoom grows), the same
        ...
        // zoom^2 reproduces that; BrightnessScale absorbs the constant
        // emitterArea + unit-gap factors (so it now means "intensity at unit
        // zoom"). Only meaningful for cone fixtures with an animated zoom;
        ...
        if (BeamProps != null)
        {
            float zoomTan = BeamProps.localEulerAngles.x;
            // Floor zoom^2 so a near-zero (pencil) beam doesn't divide to
            // infinity. 1e-4 ~= a ~1.1deg full cone -- tighter than any real
            // fixture, so it never clips a meaningful zoom.
            brightness /= Mathf.Max(zoomTan * zoomTan, 1e-4f);
        }
    ...
    void UpdateRange(DiamondFixtureDefinition fixture)
    {
        ...
        point.cutoff = DiamondBeamMath.DeriveRoundBeamLength(
            emitterRadius, zoomTan, poolReachIntensity, haze, cutoffThreshold, beamLengthMax);
    }

    // Mirrors animated zoom onto a Bakery cone light's angle. No-op unless the
    ...
    void UpdateConeAngle()
    {
        ...
        point.angle = DiamondFixtureDefinition.ZoomTanToDegrees(tanHalfAngle);
    }
