...
using GroupEntry         = DiamondFixtureMapLayout.GroupEntry;
...
public class DiamondEWinFixtureMap : EditorWindow
{
    ...
    // Data types (FixtureEntry, GroupEntry, FixtureLayout, GroupLayout, SelectionGroup, etc.)
    // are defined in DiamondFixtureMapLayout.
    ...
    // Used to load sibling assets (UXML, theme JSON) without hardcoding folder paths.
    private static string ScriptDir()
    {
        foreach (var guid in AssetDatabase.FindAssets($"t:MonoScript {nameof(DiamondEWinFixtureMap)}"))
        {
            string path = AssetDatabase.GUIDToAssetPath(guid);
            if (path.EndsWith($"{nameof(DiamondEWinFixtureMap)}.cs"))
                return Path.GetDirectoryName(path).Replace('\\', '/');
        }
        return "Assets/Modules/Diamond/Editor";
    }

    private static string ThemePath => $"{ScriptDir()}/MapTheme.json";
    ...
    // --- Lifecycle ---------------------------------------------------
    [MenuItem("Tools/Diamond/Fixture map")]
    private static void Open() => GetWindow<DiamondEWinFixtureMap>("Fixture map");
    ...
    public void CreateGUI()
    {
        ...
        // Set window icon
        string iconPath = $"{dir}/Resources/Icons/Icon Diamond EWin FixtureMap@2x.png";
        Texture2D windowIcon = AssetDatabase.LoadAssetAtPath<Texture2D>(iconPath);
        titleContent = new GUIContent("Fixture map", windowIcon);

        // Create UXML layout
        var uxml = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>($"{dir}/DiamondEWinFixtureMap.uxml");
        if (uxml == null)
        {
            rootVisualElement.Add(new Label($"DiamondEWinFixtureMap.uxml not found in {dir}."));
            return;
        }
    ...
    private void LoadTheme()
    {
        ...
        {
            Debug.LogWarning($"[Diamond] Failed to load theme {ThemePath}: {ex.Message}");
        }
    }
    ...
    {
        var result = DiamondFixtureMapLayout.ComputeLogicalLayout(
            _fixtures, _groups,
            _minGap, _gapCompressionK, _nodeCompressionK, _nodeCompressionThreshold, _flipY);
        _fixtureLayouts = result.fixtureLayouts;
        _groupLayouts   = result.groupLayouts;
        _logicalBounds  = result.logicalBounds;
    }
    ...
    private void RefreshSgList()
    {
        ...
        {
            int idx = i;
            var item = new DiamondSelectionGroupItem();
            item.GroupIndex = idx;
    ...
    private void RefreshSgListSelectionStates()
    {
        ...
        {
            if (_sgList[i] is DiamondSelectionGroupItem item && item.GroupIndex >= 0 && item.GroupIndex < _selectionGroups.Count)
            {
