...
public class DiamondEWinFixtureProperties : EditorWindow
{
    ...
    private static void Open() => GetWindow<DiamondEWinFixtureProperties>("Fixture properties");
    ...
    {
        foreach (var guid in AssetDatabase.FindAssets($"t:MonoScript {nameof(DiamondEWinFixtureProperties)}"))
        {
            string path = AssetDatabase.GUIDToAssetPath(guid);
            if (path.EndsWith($"{nameof(DiamondEWinFixtureProperties)}.cs"))
    ...
    public void CreateGUI()
    {
        ...
        // Set window icon
        string iconPath = $"{dir}/Resources/Icons/Icon Diamond EWin FixtureProperties@2x.png";
        ...
        // Create UXML layout
        var uxml = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>($"{dir}/DiamondEWinFixtureProperties.uxml");
        if (uxml == null)
        {
            rootVisualElement.Add(new Label($"DiamondEWinFixtureProperties.uxml not found in {dir}."));
            return;
        }
