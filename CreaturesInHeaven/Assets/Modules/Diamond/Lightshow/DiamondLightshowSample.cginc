...
// DiamondBeamCommon.cginc) and the lamp glow (DiamondLampGlow.shader) -- so the texture
// globals, the addressing math, and the frame lerp can never drift between them. Before
// this was extracted, the beam cginc and the glow shader each carried their own copy of
// DiamondLoadTexel + a sampler, and a change to one (e.g. the frame-lerp or the colour-row
// index) would silently desync the lamp glow from the beam colour. Now there's one copy.
//
...
// The current frame column is _UdonDiamondLightshowFrames[_ShowIndex]; callers lerp the
// two bracketing columns for smoothness (bake fps < render fps).
#ifndef DIAMOND_LIGHTSHOW_SAMPLE_INCLUDED
#define DIAMOND_LIGHTSHOW_SAMPLE_INCLUDED

#ifdef DIAMOND_LIGHTSHOW_TEX

// These are set from Udon via VRCShader.SetGlobal*, so they MUST start with _Udon --
// VRChat blocks Udon from writing any global shader property outside that namespace.
// Prefixed with _UdonDiamondLightshow (not just _UdonLightshow) to avoid colliding with
// any other world/package's own _Udon-namespaced globals. (Per-instance _FixtureRow /
// _ShowIndex are set via MaterialPropertyBlock, not globals, so they keep plain names and
// are declared by each shader's own instancing buffer, not here.)
...
// Reads a fixture's full animated state (colour + beam channels) at the current
// fractional frame. Colour is the same value DiamondSampleColour returns; the beam row
// adds zoom/focus/intensity. The beam intensity is HDR-de-scaled but NOT master-scaled
// here -- the caller folds in _UdonDiamondBeamIntensityScale so it flows through the
// early-out and beam-length derivation, matching the proxy path.
