# Diamond fixture rig manager: restructuring plan
...
The fix for is a central rig manager: one UdonBehaviour with one `Update()` that drives
all fixtures. Beyond performance, a rig root turns out to be the natural home for several
existing and future functionality that currently sit awkwardly or have no clear home, such as
rig-wide atmosphere (haze) control, FixtureMap data for editor windows, multi-rig organisation,
presets, and future DMX/MIDI control.

## Runtime model

The model is **data-oriented**: one rig behaviour owns the per-fixture object graph as
parallel arrays and does every per-frame work in a single `Update()` loop. In ECS terms:

- **Rig = the World + System.** Owns the arrays (components), runs the single `Update()`
...
work. Time-slicing, frame-budgeting, batching, or event-driven update models will need the rig
to see and control the whole fixture set as data rather than delegate to autonomous per-fixture
objects.

### Rig responsibilities

The rig is a **monolithic single-source-of-truth replacement for 600 per-fixture drivers**.
...
Per-fixture data has four distinct owners, and the rig holds only the first:

1. **Object graph → rig arrays.** The references the loop and tooling touch: fixture root,
...
`BeamProps.localScale.y` = beam intensity, `Head.localRotation` directly. The rig is the sole
...
static config the rig has to bake and hold. Wiring this is a stage-1 prerequisite.

Because the proxy transforms are the fixed, animator-driven input, they double as a
correctness oracle: the rig's output for a given clip can be validated against the proxy
...
- **`DiamondFixtureRig`** (new, runtime UdonSharpBehaviour) — the World/System. Marks a rig
  root, owns the parallel fixture arrays, runs the single `Update()` loop, holds rig-wide
  runtime atmosphere values, and is the eventual entry point for presets / DMX / MIDI.
- **`DiamondFixtureRigDefinition`** (new, editor MonoBehaviour, stripped at build) — the
  authoring/serialisation layer. Rig metadata (`DisplayName`), fixture map data, authored rig-wide
  values, and the bake step that crawls fixtures and populates the rig's arrays at edit time. This is
...
flag) lives as a row in the rig's arrays, and all per-fixture *logic* runs in the rig's loop.
`DiamondFixtureDefinition` remains as the per-fixture edit-time authoring component, unchanged
in spirit; the rig bake reads from it (and from the material) to compute the edit-time bounds,
...
> behaviour, which is retired once the rig loop reaches playback parity (stage 1+2).

## Staged migration

Ordered so each stage is independently testable, and so the highest-risk assumption —
"the rig can drive a real animation clip correctly" — is proven before any tooling is
touched. Stages 1 and 2 form a single indivisible milestone (an empty rig holding arrays has
no observable pass/fail; the test *is* a fixture lighting correctly from the arrays).

### Stage 0 — Colour keying prerequisite

1. **Emission colour proxy.** Emission colour is currently static and unanimatable. Give it
   its own proxy transform channel so the rig reads it in the loop like any other animated value,
   rather than baking/holding it as per-fixture data.

### Stage 1 — Rig runtime + driver replacement

Build the rig's arrays and the per-entity read/apply loop together.

1. **Arrays (object graph).** The rig holds one aligned array per reference channel:
...
3. **Bounds (bake-time, not runtime).** The culling AABB is set once, so compute it at bake
   ...
   `Renderer.bounds`. No bounds scalars live on the rig at runtime.
4. **Populate (interim).** A minimal edit-time bake that crawls the rig root for fixtures and
...
5. **Exit criterion (the whole point of this milestone):** an existing animation clip plays
   through the rig with visually correct lighting, validated against the proxy-transform
...
write fixture state from the rig's arrays by index. Expected to be a small change: these
tools already read material and object properties directly off each fixture object; the added
work is resolving a selected fixture to its rig index and reading/writing there. Verify
per-fixture editing still works.

### Stage 3 — FixtureMap centralisation + stable identity

Fold FixtureMap data onto the rig, and solve index stability: the main new sharp edge
...
- Store the FixtureMap either as an **associated sidecar file** keyed by identity, or
  **serialised directly** into a rig script. (Open decision — see below.)

## Rig-wide atmosphere (folds into the loop, post-milestone)
...
`_ScatterStrength`, `_Anisotropy`). In the data-oriented model they become rig-level values
...
- **Flat, set once at Start.** A serialized float on the rig, applied across the rig at
  startup. Cheapest; right if atmosphere is a static per-scene property.
- **Animatable (proxy on the rig).** Give the rig its *own* proxy transforms and read them in
...
- **Bounds implication.** The culling AABB is sized from worst-case haze/scatter. If the rig
  drives atmosphere at runtime, the bake-time bounds (stage 1) must be sized to the rig's
...
- **Time-slicing / frame-budgeting** — the rig owns the loop and a cursor, so it can process
  a range of fixtures per frame to flatten the body-work spike.
- **Batching** — SoA layout lets the rig see across fixtures to group shared work.
- **Event-driven updates** — rig-level indices (by group/state) let a change touch only the
  affected fixtures.
- **Multiple rigs** — each rig is one World with one `Update`; switching = enable/disable.
- **Groups** — `DiamondFixtureGroupDefinition` (just a name today) becomes an addressable
  index-subset into the rig arrays, for selective control / presets / DMX channel mapping.
- **Presets / DMX / MIDI** — external input driving fixture state through the rig (e.g. by
  writing the proxy transforms it reads, or a future per-fixture override channel). The rig is
...
- **Udon array ergonomics.** No generics / `List<T>` at runtime; per-element extern calls;
  ...
  what SoA wants anyway). The rig will be a wall of parallel arrays; the care goes into the
...
- **Atmosphere scope: rig or group?** If two haze zones in one rig are ever wanted (a hazy
  stage and a clear house), atmosphere may want to live on *groups* rather than the rig root.
  Worth deciding before the atmosphere work hardens.
- **FixtureMap storage.** Sidecar file keyed by fixture identity, or serialised directly into
  a rig script? Sidecar keeps the scene light and diffs cleanly; in-script keeps everything
  in one place but bloats the rig object.
