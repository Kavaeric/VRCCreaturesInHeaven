...
namespace VRCLightVolumes {
    ...
    public class LightVolumeSetup : MonoBehaviour {
        ...
        public void GenerateAtlas() {
            ...
            // Patch (PR #84): when a CRT post-processor is registered, the chain re-renders
            // the atlas every frame and Unity dispatches one drawcall per Z slice. Pick the
            // depth-minimising packing strategy so the per-frame cost drops accordingly,
            // even at the cost of a slightly larger atlas in W/H. See PATCHES.md.
            bool hasCrtPostProcessor = false;
            if (LightVolumeManager != null && LightVolumeManager.AtlasPostProcessors != null) {
                foreach (var pp in LightVolumeManager.AtlasPostProcessors) {
                    if (pp != null) { hasCrtPostProcessor = true; break; }
                }
            }
            var packingStrategy = hasCrtPostProcessor
